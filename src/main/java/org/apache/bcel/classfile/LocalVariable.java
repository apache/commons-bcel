...
public final class LocalVariable implements Cloneable, Node, Constants {
    ...
    private int original_index; // never changes; used to match up with LocalVariableTypeTable entries
    ...
    public LocalVariable(final LocalVariable c) {
        this(c.getStartPC(), c.getLength(), c.getNameIndex(), c.getSignatureIndex(), c.getIndex(),
                c.getConstantPool());
        this.original_index = c.getOriginalIndex();
    }
     ...
     * @param index Variable is `index'th local variable on the method's frame
     * @param constant_pool Array of constants
     * @param original_index Variable is `index'th local variable on the method's frame prior to any changes
     */
    public LocalVariable(final int start_pc, final int length, final int name_index, final int signature_index, final int index,
            final ConstantPool constant_pool, final int original_index) {
        this.start_pc = start_pc;
        this.length = length;
        this.name_index = name_index;
        this.signature_index = signature_index;
